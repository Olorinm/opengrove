...
App 可以在 manifest 里声明默认群聊员工。OpenGrove 只读取这些通用声明，不再根据某个具体 App 的 id 或标题推断员工。
...
同一个数组也可以放在 `capabilities.employees`。OpenGrove 创建 room member 时会把 employee id 作用域限定在 App id 下。
