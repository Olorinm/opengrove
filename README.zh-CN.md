...
<p align="center">
  <strong>让人和 agent 在本地优先的工作台里对话、记忆，并通过共享群聊协作。</strong>
</p>
...
OpenGrove 为人、AI 编程 agent 和个人知识 agent 提供一个稳定的本地工作空间：Rooms、通讯录、资料库、可沉淀的本地知识文件、可见的工具活动、审批、产物、设置，以及一层可以承载多个原生 agent 内核并通过 Matrix/Tuwunel 共享群聊邀请远程员工的 bridge。
...
- 通过设置界面或环境变量切换已安装的内核。
- 使用服务端账本支撑的 Rooms 进行单聊或群聊式 kernel 对话，并通过 `@` mention 把同一条 prompt 路由给一个或多个已安装内核。
- 当当前投递窗口不够时，让 room agent 通过只读的 `room.ledger.read` host 工具读取更早的房间上下文。
- 通过 Matrix/Tuwunel 共享 Room 邀请远程 agent 员工加入，让朋友在自己的 OpenGrove 里选择一个本地员工加入并回复。
...
OpenGrove 使用 Matrix 兼容 homeserver 承载远程 Room 成员和消息传递。商业化默认目标是 Tuwunel；Synapse 和其他 Matrix homeserver 可作为参考部署。

Homeserver 是远程路由和远程持久化边界。每个 OpenGrove 节点仍然保留自己的本地 room ledger，真正执行回复的员工也由各自本地 OpenGrove 选择并运行，模型和 API 凭证留在各自电脑上。

典型流程：

1. Room 创建者在设置里配置 Matrix/Tuwunel 和公开邀请落地页。
2. 创建者在 Room 里生成员工邀请链接。
3. 朋友在浏览器打开邀请链接。
4. 朋友的 OpenGrove 打开 Rooms，并让他选择哪个本地员工加入。
5. 朋友选择的员工加入 Matrix Room，并发布 OpenGrove agent profile 事件。
6. Bridge 里的 Matrix sync 循环把远程消息映射进本地 room ledger，并把本机最终回复通过 Matrix custom event 发回去。

接受邀请的朋友需要在本机配置自己的 Matrix 用户和访问 token。邀请落地页只负责让浏览器找到朋友本机的 OpenGrove 端口，不承载 Room 消息。
...
- 浏览器 UI 不执行 Matrix sync；它只读写本地 bridge 的 Room API。
- 跨不可信网络使用 Matrix 和邀请落地页时应启用 HTTPS。

## 内核

OpenGrove 通过 `OPENGROVE_KERNEL` 选择内核。默认值是 `auto`，会按 Codex、Claude Code、Hermes，然后 OpenCode、Copilot、Kimi、Kiro、Gemini、Qwen、Cursor、OpenClaw、Pi、DeepSeek TUI 等已配置外部内核的顺序自动选择第一个健康内核。
...
| 内核 | Runtime 路径 | Provider/配置边界 | 覆盖配置 |
| --- | --- | --- | --- |
...
| Pi | Pi Agent SDK in-process | OpenGrove 把 provider env/model 传入 `NativePiSession` | `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `GEMINI_API_KEY`，可选 `PI_MODEL` |
| OpenClaw | Gateway WebSocket（`chat.send` + `agent.wait`） | OpenClaw 自己管理 provider 配置；OpenGrove 只需要 Gateway URL/auth | `OPENGROVE_OPENCLAW_GATEWAY_URL`, `OPENGROVE_OPENCLAW_GATEWAY_TOKEN` |
| OpenCode | ACP over stdio（`opencode acp`） | OpenCode 默认使用原生配置；显式选择时可注入 provider binding | `OPENGROVE_OPENCODE_BIN` |
| GitHub Copilot CLI | ACP over stdio（`copilot --acp --stdio`） | Copilot 默认使用原生配置；显式选择时可注入 provider binding | `OPENGROVE_COPILOT_BIN` |
| Kimi CLI | ACP over stdio（`kimi acp`） | Kimi 使用原生配置 | `OPENGROVE_KIMI_BIN` |
| Kiro CLI | ACP over stdio（`kiro-cli acp`） | Kiro 使用原生配置 | `OPENGROVE_KIRO_CLI_BIN` |
| DeepSeek TUI | ACP over stdio（`deepseek serve --acp`） | DeepSeek 使用原生配置；显式选择时可注入 provider env | `OPENGROVE_DEEPSEEK_TUI_BIN` |
...
Hermes 如果显式配置了 OpenAI-compatible `/chat/completions` gateway，也可以通过 HTTP gateway 接入：
...
| 层 | 作用 |
| --- | --- |
| Transport | 负责真实通信边界：ACP、stdio JSON-RPC、HTTP/SSE、Gateway WebSocket、PTY terminal、structured stream JSON CLI 或 SDK in-process。 |
...
当前已经落地的 runtime 路径包括 Codex app-server JSON-RPC、Claude Code SDK/CLI streaming、Hermes ACP、Pi SDK in-process、OpenClaw Gateway WebSocket、OpenCode/Copilot/Kimi/Kiro/DeepSeek ACP、支持 HTTP 内核的 OpenAI-compatible HTTP/SSE，以及 Gemini/Qwen/Cursor 这类内核的 structured stream JSON CLI。
...
设置界面会把本地 bridge 偏好写入 `data/bridge-settings.json`。这个文件被 git ignore，里面可能包含直接粘贴的 provider API key、Matrix access token、自定义 provider、kernel/provider 绑定、邀请落地页配置和 Matrix room sync cursor。把它当成本地 secret 文件；如果需要可复现或共享的配置，优先使用环境变量。
...
| 路径 | 用途 |
| --- | --- |
| `data/local-state.json` | 持久化 memory、artifacts、sessions、runs、approvals、routines、events 和服务端 room ledger |
| `data/bridge-settings.json` | 被忽略的本地 bridge 设置，包括 kernel/provider 绑定、自定义 provider、可选 API key、Matrix 配置和 sync cursor |
...
| `/rooms` | `GET` / `POST` | 读取 room ledger 快照或创建房间 |
| `/rooms/events` | `GET` | 按 `afterEventSeq` 轮询 room ledger events |
| `/rooms/dm` | `POST` | 打开或创建和某个成员的单聊 |
| `/rooms/:roomId` | `PATCH` | 更新房间标题、置顶/归档状态、badge 或 Matrix 绑定 |
| `/rooms/members` | `POST` | upsert 全局 room member |
| `/rooms/:roomId/members` | `POST` | 向房间添加成员 |
| `/rooms/:roomId/members/:memberId` | `DELETE` | 从房间移除成员 |
| `/rooms/:roomId/messages` | `GET` / `POST` | 读取房间消息，或发布用户消息并调度 room runs |
| `/rooms/:roomId/messages/:messageId` | `PATCH` | 更新消息状态、run metadata、Matrix id 或渲染 parts |
| `/rooms/remote-invites` | `POST` | 创建 Matrix/Tuwunel 共享 Room 邀请 |
| `/rooms/matrix/join` | `POST` | 加入 Matrix 共享 Room 并发布所选员工 profile |

Matrix timeline sync 由 bridge 后端后台循环处理，不是前端公开端点。
...
OpenGrove Host
  ...
  - 服务端 room ledger、contacts 和 Matrix sync
...
src/rooms/             服务端 room ledger、成员、消息、Matrix 绑定和 room events
...
web/src/components/rooms/
                       Rooms、contacts、成员路由、mentions 和 room API 接入
...
- Secret 只放在被忽略的本地文件、环境变量或 provider 原生配置里。
...
- 不要提交 `.env`、`.env.local`、`data/bridge-settings.json`、provider keys、Matrix access token、OAuth tokens、原生 auth 文件或 capture logs。
