...
Homeserver 是路由和持久化边界。真正执行回复的员工仍然由各自本地 OpenGrove 选择并运行，模型和 API 凭证留在各自电脑上。
...
6. Room 消息和最终回复通过 Matrix custom event 传递。
...
| `/rooms/matrix/events` | `POST` | 发布 OpenGrove Matrix room event |
| `/rooms/matrix/sync` | `GET` | 同步 Matrix room state 和 timeline event |
